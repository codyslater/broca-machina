...
| `transport.transcriptDir` | string | — (required for `file`) | Directory each transcript is written to as `<timestamp>.txt`. *(mcp: optional tee of every transcript for an external router.)* |
| `transport.replyFile` | string | — (required for `file`) | File polled for reply text; consumed (deleted) after it's spoken. *(mcp: optional additional voice source, e.g. for a router or the SSH shuttle.)* |
...
| `semanticEndpoint` | object | `{"enabled": true, "holdMs": 4000}` | A transcript that sounds unfinished (trailing comma, or a dangling connective read from Whisper's own punctuation) is **held** `holdMs` and joined with a continuation spoken inside the window; none coming, it flushes as-is. A period after a preposition or object pronoun is trusted as an end. `enabled: false` = every pause is a turn. *optional* |
| `maxCaptureSec` | number | `60` | Capture watchdog: a capture that outlives this is force-finalized, so a decoder crash or a stream that never ends costs one utterance, not the session. `0` disables. *optional* |
| `logFile` | string \| null | `null` | Persistent copy of every log line with full ISO timestamps (an MCP host keeps stderr only sporadically); rotated to `.1` at boot once it passes 5 MB. *optional* |
| `localAck` | object | `{"enabled": false, "ollamaHost": "http://localhost:11434", "model": "gemma4:e4b", "numPredict": 30, "timeoutMs": 4000, "fireAfterMs": 2000, "contextTurns": 6, "maxWords": 12, "systemPrompt": …}` | Content-aware filler from a warm local Ollama model: generated right after STT, played at `fireAfterMs` only if the real reply has not landed (one filler per turn; the canned `ackPhrase` tier is the same-moment fallback). Trimmed to one sentence of `maxWords`; an ack that claims an action is dropped. Fail-open on any model or TTS failure. *optional* |
| `thinkingEarcon` | object | `{"file": null, "afterMs": 4000, "volume": 0.15, "maxMs": 45000}` | Ambient "still working" loop that starts `afterMs` after the turn's filler while the reply is still pending (`assets/thinking.wav` ships). Cut by the reply or confirmed speech, muted while a speaking edge awaits its verdict, gives up after `maxMs`. Off until `file` is set. *optional* |
| `longWaitNotice` | object | `{"afterMs": 0, "who": "the brain", "phrase": [...]}` | One spoken line `afterMs` into a turn whose reply still has not landed; `{who}` in a phrase is replaced by the routed session's name from `voiceFallback.speakerRouteFiles`, else `who`. `0` disables. *optional* |
| `addressGate` | object | `{"enabled": false, "wakeWords": ["assistant"], "hotWindowMs": 300000, "fuzzy": true, "ollamaHost": …, "model": …, "timeoutMs": 2500}` | Drops speech the enrolled speaker directs at other people in the room. A wake word (fuzzy: within one edit) delivers instantly; anything inside `hotWindowMs` of the assistant finishing talking, or while a reply is still owed, is a reply by construction; only a cold turn is judged ADDRESSED/ASIDE by the local model with recent turns as context. Asides are teed to `transcriptDir` as `.aside`, never forwarded. Fail-open. *optional* |
| `speakerGate` | object | `{"refFile": null, "enrollDir": null, "enrollUtterances": 8, "buildCmd": null, "enrollMinSilenceMs": 1500, "duckCheckMs": 0, "duckCheckScore": null}` | Respond to **one** enrolled voice. `refFile` configured but absent starts a spoken enrollment conversation on join (nothing said during it reaches a brain), builds the voiceprint with `buildCmd`, then arms; enforcement lives in the STT layer via `VOICE_SPEAKER_REF` / `VOICE_SPEAKER_THRESHOLD` in `stt.env`. `duckCheckMs > 0` scores the first slice of a capture against the voiceprint before dimming playback, so the bot's own audio in an open mic never ducks a reply (needs the warm STT server). Biometric data: keep `refFile`/`enrollDir` outside the repo. *optional* |
| `transport.gateFile` | string \| null | `null` | *(mcp)* While this file exists the channel notification is suppressed and the transcript is only teed to `transport.transcriptDir`: a router owns delivery and answers through `transport.replyFile`, which is polled as an additional voice source on any transport. *optional* |
